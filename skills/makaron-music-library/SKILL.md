...
4. Use `musiclib recommend --request`, or pass `--scene kpop-stage` or `--scene ecommerce`. Remote video upload is not available in 0.2.2; describe the video with `--brief` or `--request`.
...
- Return `REMOTE_VIDEO_UPLOAD_NOT_IMPLEMENTED` as a request for a textual video brief in 0.2.2.
